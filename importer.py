...
# assumes pbf file in /vagrant/mapping.json
mapping = os.path.abspath(os.path.sep + 'vagrant' + os.path.sep + 'mapping.json')

# assumes pbf file in /vagrant/data/
pbf = os.path.abspath(os.path.sep + 'vagrant' + os.path.sep + 'data' + os.path.sep + 'germany-latest.osm.pbf')

# assumes imposm3 in root of project
imposm_path = '/Users/jvf/Code/spatial_db_project/spatial-weather'
...
def import_from_pbf():
    ...
    # because imposm3 is running inside the vm the port is different than accessing the postgres server from outside the vm
    db_port = 5432


    connection = 'postgis://' + db_user + ':' + db_pw + '@' + db_host + ':' + str(db_port) + '/' + db_name
    os.environ["PATH"] += ":" + imposm_path
    try:
